# code is from https://github.com/AntoineBellemare/DAT_GPT/blob/main/
# metric is added as an argument in analyze_results()
...
def analyze_results(results_df, variable, order, metric='Score'): 
    variables = order
    n_variables = len(results_df[variable].unique())
    # Calculate mean and confidence interval for each model's distribution
    mean_conf = results_df.groupby(variable)[metric].agg(["mean", "median", "sem", "std"])
    ...
    for i, model1 in enumerate(variables):
        for j, model2 in enumerate(variables):
            if i >= j:
                continue
            score1 = results_df[results_df[variable] == model1][metric].dropna()
            score2 = results_df[results_df[variable] == model2][metric].dropna()
            res = ttest_ind(
                score1,
                score2,
...
def create_heatmap(
    ...
    # Remove grid lines from bar plot
    axs[0].grid(False)
    ...
    # Prepare heatmap annotations with smaller font size
    mask = mask[1:, :-1]
    ...
    # Create heatmap with smaller annotations
    sns.heatmap(
        heatmap_data,
        mask=mask,
        ...
        annot_kws={'size': annot_size}  # Made annotations even smaller
    )

    # Make tick labels smaller as well for consistency
    axs[1].tick_params(labelsize=12)  # Reduced from 16
    ...
    colorbar.ax.tick_params(labelsize=10)  # Reduced from 12
    colorbar.set_label(cbar_label, fontsize=12)  # Reduced from 14
    axs[1].set_xlabel("")
    axs[1].set_ylabel("")
    axs[1].grid(False)
