"""
This code was adapted from Bellemare-Pepin, DAT_GPT: https://github.com/AntoineBellemare/DAT_GPT/tree/main
"""
import pandas as pd
import numpy as np
...
def analyze_results(results_df, variable, order, metric='Score'): # Added metric as an argument 
    variables = order
    ...
    for i, model1 in enumerate(variables):
        for j, model2 in enumerate(variables):
            ...
            if len(score1) == 0 or len(score2) == 0:
                print(f"Warning: No data for comparison between {model1} and {model2}")
                pvals[i, j] = np.nan
                pvals[j, i] = np.nan
                tvals[i, j] = np.nan
                tvals[j, i] = np.nan
            ...
            # Compute Cohen's d only if we have data
            tval = res.statistic
...
def create_heatmap(
    ...
    # Add error bars and significance stars
    ...
    axs[1].set_yticklabels(axs[1].get_yticklabels(), rotation=0, fontsize=14)
